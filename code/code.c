...
void modulo()  /* resto de la division entera del segundo valor de la pila por el valor de la cima */
{
...
void concatenacion()
{
  ...
  d1.cadena[strlen(d1.cadena) - 1] = '\0';
...
void preincremento()
{
  ...
  if (d.sym->tipo != VAR && d.sym->tipo != INDEFINIDA)
    execerror(" asignacion a un elemento que no es una variable ", d.sym->nombre);
...
void leervariable_cadena()
{
 ...
 /* Se comprueba si el identificador es una variable */
  if ((variable->tipo == INDEFINIDA) || (variable->tipo == VAR))
    {
    while ((aux = getchar()) != '\n' && aux != EOF); //Borrar buffer de entrada.
    printf("Cadena--> ");
    fgets(c,1024,stdin);
...
void escribir_cadena()
{
  ...
  if(d.cadena != NULL){
    int i;
    for(i=0;i<strlen(d.cadena)-1;i++){
      if(d.cadena[i]=='\'') continue;

      if(d.cadena[i]=='\\'){
        if(d.cadena[i+1]=='n'){
          printf("\n");
          i++;
        }else if(d.cadena[i+1]=='t'){
          printf("\t");
          i++;
...
void forcode()
{
 ...
 for(variable->u.val=d1.val; variable->u.val<=d2.val; variable->u.val=variable->u.val+d3.val)
  {
...
void add()
{
 ...
 if (d1.sym->tipo != VAR && d1.sym->tipo != INDEFINIDA)
  ...
  d1.sym->u.val=d1.sym->u.val + d2.val;   /* Asignar valor   */
  d1.sym->tipo=VAR;
